...
void buildCommits(char* line, int lineNumber, GTree *t) {
    char* buff2 = line;
    ...
    char* author_key = malloc(sizeof(char)*200);
    char* commiter_key = malloc(sizeof(char)*200);
    temp->date = data_commit;

    strcpy(author_key, author_id);
    strcpy(commiter_key, commiter_id);
