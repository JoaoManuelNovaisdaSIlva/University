...
def minimax(tabuleiro, profundidade, BOT):
    copia = tabuleiro
    dic = {}
    i = 0
    while i<= profundidade:
        for vazias in celulas_vazias(copia):
            dic[vazias] = list()
            copia[vazias[0]][vazias[1]] = escolha_bot
            if verifica_vitoria(copia, BOT):
                return vazias
            #else: copia = tabuleiro
            for vazias2 in celulas_vazias(copia):
                dic[vazias] = vazias2
                copia[vazias2[0]][vazias2[1]] = escolha_humano
            copia = tabuleiro
        i += 1
