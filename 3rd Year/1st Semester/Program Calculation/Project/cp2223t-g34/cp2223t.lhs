...
Como falamos nas aulas, a sequência de Fibonacci é um hilomorfismo do tipo LTree.
Esta árvore é construída de maneira que o número de folhas seja igual a um
número da sequência. É evidente que o processo de gerar n folhas para cada
algarismo da sequência torna, a sua geração, ineficiente. Mas podemos otimizar a função
recorrendo á lei de recursividade mútua.

Para tal, deveremos começar por verificar quantas "funções escondidas"
existem na definição da função original. Ora, como este apresenta 3 
casos base e o parâmetro da função é (n+3) temos que existe a possibilidade de
repartirmos fib em 3 funções.
Com o objetivo de reduzir o parâmetro vamos definir uma nova função:

            f1'' a b c n = f1 a b c (n+2)

Sendo assim, temos também que:

            f1'' a b c (n+1) = f1 a b c (n+3)

E como f1 a b c (n+3) está definida como:

f1 a b c (n+3) = a*f1 a b c (n+2) + b*f1 a b c (n+1) + c*f1 a b c n

Logo, f1'' a b c (n+1) = a*f1 a b c (n+2) + b*f1 a b c (n+1) + c*f1 a b c n

Portanto, temos:

            f1'' a b c 0 = 0
            f1'' a b c (n+1) = a*f1'' a b c n + b*f1 a b c (n+1) + c*f1 a b c n

            f1 a b c 0 = 0
            f1 a b c 1 = 1
            f1 a b c (n+2) = f1'' a b c n

Agora podemos repetir o processo para f1:

            f1' a b c n = f a b c (n+1)
            f1' a b c (n+1) = f a b c (n+2)

Sendo assim, temos:

            f1 a b c 0 = 0
            f1 a b c (n+1) = f1' a b c n

            f1' a b c 0 = 1
            f1' a b c (n+1) = f1'' a b c n

            f1'' a b c 0 = 1
            f1'' a b c (n+1) = a*f1'' a b c n + b*f1' a b c n + c*f1 a b c n

Com isto, podemos agora, transformar estas funções mutuamente recursivas para 
igualdades pointfree:

           f1 . either (const 0) succ = either (const 0) f1'
           f1' .  either (const 0) succ = either (const 1) f1''
           f1'' . either (const 0) succ = either (const 1) (add . (split f1 f1' f1''))

Como in = either (const 0) succ
E usando, também, a lei de absorção + revertida temos que:

           f1 . in = either (const 0) p1 . F (split f1 f1' f1'')
           f1' . in = either (const 1) p2 . F (split f1 f1' f1'')
           f1'' . in = either (const 1) add . F (split f1 f1' f1'')

Onde temos um functor F f1 = id + f1
Agora podemos aplicar a lei Fokkinga

           (split f1 f1' f1'') = cata (split (either (const 0) p1) (either (const 1) p2) (either (const 1) add))

Com a lei de troca:

           (split f1 f1' f1'') = cata (either (split (const 0) (const 1) (const 1)) (split p1 p2 add))

Passando para para pointwise e definindo (split f1 f1' f1'') como fbl'


\begin{code}
loop a b c (f1, f1', f1'') = (f1', f1'', a*f1'' + b*f1' + c*f1)
initial = (0,1,1)
wrap (a,b,c) = a
...
f1'' a b c (n+1) = a*f1'' a b c n + b*f1' a b c n + c*f1 a b c n
\end{code}
Agora para testes de desempenho usamos o módulo System.CPUTime para
determinar o tempo que cada função demora a gerar, neste caso, 1 milhão de números da sequência
\begin{code}
callNTimes f n = map f [1..n]

aux f = callNTimes (\i -> f 1 1 1 i) (10^6)
...
compareFunctions = do 
  ...
  putStrLn $ "A funcao f demorou: " ++ (show time1) ++ " segundos"

  startTime2 <- getCPUTime
  let result2 = aux fbl 
  endTime2 <- getCPUTime
  let time2 = (fromIntegral (endTime2 - startTime2)) / (10^12)
  putStrLn $ "A funcao fbl demorou: " ++ (show time2) ++ " segundos" 
\end{code}
...
carpets = anaList fcarpets

fcarpets 0 = i1 []
fcarpets x = i2 ((sierpinski (((0,0), 32), x)), x-1)

present = cataList fpresent
fpresent = either presentIOnil presentIO
presentIOnil _ = return [()]
presentIO (x, _) = do 
  drawSq x
  await
  return [()]
