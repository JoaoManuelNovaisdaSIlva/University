...
public class WindowsFactory {
     ...
     */
    static public Stage aboutWindow(Window parent) throws IOException {
        Stage aboutStage = new Stage();  //
        FXMLLoader fxmlLoader = new FXMLLoader(ConverterApp.class.getResource("about-view.fxml")); // Definir o ficheiro FXML (View)
        Scene scene = new Scene(fxmlLoader.load(), 320, 240);
        aboutStage.setScene(scene);
        aboutStage.setTitle("About");
        aboutStage.initOwner(parent);
        aboutStage.setAlwaysOnTop(true);
        aboutStage.setMaxHeight(150); // Pode ser definido no FXML - colocado aqui para demonstrar a possibilidade
        aboutStage.setMaxWidth(200);  // Pode ser definido no FXML - estes valores têm precedência sobre os definidos no FXML
